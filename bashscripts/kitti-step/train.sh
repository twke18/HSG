...
# the baseline method with HSG on KITTI-STEP without
# ground-truth annotations. Users could also modify from this
# script for their use case.
#
# Usage:
#   # From HSG/ directory.
#   source bashscripts/kitti-step/train.sh
#
#
...
INFERENCE_SPLIT=val
INFERENCE_IMAGE_SIZE=1242
INFERENCE_CROP_SIZE_H=375
INFERENCE_CROP_SIZE_W=1242
INFERENCE_STRIDE=1242

# Set up path for saving models.
SNAPSHOT_DIR=snapshots/kitti-step/fcn_res50_hsg
echo ${SNAPSHOT_DIR}
...
DATAROOT=/home/twke/data/KITTI-STEP/rearranged
TRAIN_DATA_LIST=datasets/kitti-step/${TRAIN_SPLIT}_pmi.txt
TEST_DATA_LIST=datasets/kitti-step/${INFERENCE_SPLIT}.txt
MEMORY_DATA_LIST=datasets/kitti-step/${TRAIN_SPLIT}_pmi.txt
...
if [ ${IS_TRAIN_1} -eq 1 ]; then
  ...
  python3 pyscripts/benchmark/benchmark_by_mIoU.py\
    ...
    --gt_dir ${DATAROOT}/kitti-step/semantic_maps/val\
    --num_classes 19
