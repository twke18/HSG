...
def gather_and_reorder_image_indices(image_indices, anchor_device=None):
  ...
  image_indices = scatter_gather.gather(image_indices, anchor_device)

  # Re-order image index based on occurring order.
  _, image_indices = torch.unique(
      image_indices, return_inverse=True) # N
  one_hot = common_utils.one_hot(image_indices) # N x M
  inds = torch.arange(1, image_indices.shape[0]+1,
                      dtype=torch.long,
                      device=image_indices.device)
  one_hot *= inds.view(-1, 1)
  one_hot = one_hot.masked_fill(one_hot == 0, inds.max())
  occ_order, _ = torch.min(one_hot, dim=0) # M
  occ_order = occ_order - 1
  image_indices = torch.gather(occ_order, 0, image_indices) # N
  _, image_indices = torch.unique(image_indices, return_inverse=True)

  image_indices = [image_indices.to(d) for d in devices]

  return image_indices



def gather_and_update_cluster_mappings(cluster_indices_1,
                                       cluster_indices_2,
                                       anchor_device=None):
  ...
  # Gather all datas from each gpu to the first one.
  cluster_indices_1 = scatter_gather.gather(cluster_indices_1,
                                            anchor_device)
  cluster_indices_2 = scatter_gather.gather(cluster_indices_2,
                                            anchor_device)

  # Compute mapping.
  max_ind = cluster_indices_2.max() + 1
  mapping = cluster_indices_1 * max_ind + cluster_indices_2
  mapping = torch.unique(mapping)
  cluster_mapping = torch.zeros(cluster_indices_1.max() + 1,
                                dtype=torch.long,
                                device=anchor_device)
  max_ind = max_ind.detach()
  mapping = mapping.detach()
  cluster_mapping[mapping // max_ind] = mapping % max_ind
  cluster_mapping = [cluster_mapping.to(d) for d in devices]
